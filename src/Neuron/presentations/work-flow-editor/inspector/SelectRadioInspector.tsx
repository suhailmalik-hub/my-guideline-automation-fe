...
export const SelectRadioInspector = ({ step, stepOrder, onUpdate, onDelete }: StepInspectorProps) => {
  ...
  const handleSave = () => {
    try {
      ...
      console.log('[Neuron] Select Radio step saved:', output);
    } catch (error) {
      console.error(`Failed to save Select Radio step: ${error instanceof Error ? error.message : error}`);
    }
  };
    ...
    >
      ...
      >
        <input
          ...
          placeholder='e.g. Select Yes/No Option'
      ...
      >
        <textarea
          ...
          placeholder='Describe the radio button to select…'
      ...
      >
        <input
          ...
          placeholder='e.g. Radio option value'
