...
export const SelectDropdownInspector = ({ step, stepOrder, onUpdate, onDelete }: StepInspectorProps) => {
  ...
  const handleSave = () => {
    try {
      ...
      console.log('[Neuron] Select Dropdown step saved:', output);
    } catch (error) {
      console.error(`Failed to save Select Dropdown step: ${error instanceof Error ? error.message : error}`);
    }
  };
    ...
    >
      ...
      >
        <textarea
          ...
          placeholder='Describe the dropdown to select from…'
      ...
      >
        <input
          ...
          placeholder='e.g. Option to select'
