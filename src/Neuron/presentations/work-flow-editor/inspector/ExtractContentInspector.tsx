...
export const ExtractContentInspector = ({ step, stepOrder, onUpdate, onDelete }: StepInspectorProps) => {
  ...
  const handleSave = () => {
    try {
      ...
      console.log('[Neuron] ExtractContent step saved:', output);
    } catch (error) {
      console.error(`Failed to save ExtractContent step: ${error instanceof Error ? error.message : error}`);
    }
  };
    ...
    >
      ...
      >
        <input
          ...
          placeholder='e.g. Extract Page Content'
      ...
      >
        <textarea
          ...
          placeholder='Describe what content to extract…'
